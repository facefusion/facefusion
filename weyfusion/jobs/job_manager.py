...
from weyfusion.date_helper import get_current_date_time
from weyfusion.filesystem import create_directory, is_directory, is_file, move_file, remove_directory, remove_file, resolve_file_pattern
from weyfusion.jobs.job_helper import get_step_output_path
from weyfusion.json import read_json, write_json
from weyfusion.typing import Args, Job, JobSet, JobStatus, JobStep, JobStepStatus
...
def init_jobs(jobs_path : str) -> bool:
	...
	JOBS_PATH = jobs_path
	job_status_paths = [ os.path.join(JOBS_PATH, job_status) for job_status in weyfusion.choices.job_statuses ]
...
def find_job_path(job_id : str) -> Optional[str]:
	...
	if job_file_name:
		for job_status in weyfusion.choices.job_statuses:
