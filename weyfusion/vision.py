...
from weyfusion.filesystem import is_image, is_video, sanitize_path_for_windows
from weyfusion.typing import Duration, Fps, Orientation, Resolution, VisionFrame
...
def create_image_resolutions(resolution : Resolution) -> List[str]:
	...
	if resolution:
		...
		for image_template_size in weyfusion.choices.image_template_sizes:
...
def create_video_resolutions(resolution : Resolution) -> List[str]:
	...
	if resolution:
		width, height = resolution
		temp_resolutions.append(normalize_resolution(resolution))
		for video_template_size in weyfusion.choices.video_template_sizes:
			if width > height:
