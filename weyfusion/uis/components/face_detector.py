...
from weyfusion import face_detector, state_manager, wording
from weyfusion.common_helper import calc_float_step, get_last
from weyfusion.typing import Angle, FaceDetectorModel, Score
...
def render() -> None:
	...
	if state_manager.get_item('face_detector_size') in weyfusion.choices.face_detector_set[state_manager.get_item('face_detector_model')]:
		face_detector_size_dropdown_options['choices'] = weyfusion.choices.face_detector_set[state_manager.get_item('face_detector_model')]
	with gradio.Row():
		FACE_DETECTOR_MODEL_DROPDOWN = gradio.Dropdown(
			label = wording.get('uis.face_detector_model_dropdown'),
			choices = weyfusion.choices.face_detector_models,
	...
	FACE_DETECTOR_ANGLES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = weyfusion.choices.face_detector_angles,
	...
	FACE_DETECTOR_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(weyfusion.choices.face_detector_score_range),
		minimum = weyfusion.choices.face_detector_score_range[0],
		maximum = weyfusion.choices.face_detector_score_range[-1]
	)
...
def update_face_detector_model(face_detector_model : FaceDetectorModel) -> Tuple[gradio.Dropdown, gradio.Dropdown]:
	...
	if face_detector.pre_check():
		face_detector_size_choices = weyfusion.choices.face_detector_set.get(state_manager.get_item('face_detector_model'))
...
def update_face_detector_angles(face_detector_angles : Sequence[Angle]) -> gradio.CheckboxGroup:
	face_detector_angles = face_detector_angles or weyfusion.choices.face_detector_angles
