...
from weyfusion.common_helper import calc_float_step, calc_int_step
from weyfusion.face_analyser import get_many_faces
from weyfusion.face_selector import sort_and_filter_faces
from weyfusion.face_store import clear_reference_faces, clear_static_faces
from weyfusion.filesystem import is_image, is_video
from weyfusion.typing import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
from weyfusion.uis.core import get_ui_component, get_ui_components, register_ui_component
from weyfusion.uis.typing import ComponentOptions
from weyfusion.uis.ui_helper import convert_str_none
from weyfusion.vision import get_video_frame, normalize_frame_color, read_static_image
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = weyfusion.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = weyfusion.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + weyfusion.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = ['none'] + weyfusion.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or weyfusion.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or weyfusion.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = wording.get('uis.face_selector_age_range_slider'),
				minimum = weyfusion.choices.face_selector_age_range[0],
				maximum = weyfusion.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calc_int_step(weyfusion.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calc_float_step(weyfusion.choices.reference_face_distance_range),
		minimum = weyfusion.choices.reference_face_distance_range[0],
		maximum = weyfusion.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
