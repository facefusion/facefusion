...
from weyfusion import face_masker, state_manager, wording
from weyfusion.common_helper import calc_float_step, calc_int_step
from weyfusion.typing import FaceMaskRegion, FaceMaskType, FaceOccluderModel, FaceParserModel
...
def render() -> None:
	...
	with gradio.Row():
		FACE_OCCLUDER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = weyfusion.choices.face_occluder_models,
		...
		FACE_PARSER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = weyfusion.choices.face_parser_models,
	...
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = weyfusion.choices.face_mask_types,
	...
	FACE_MASK_REGIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = weyfusion.choices.face_mask_regions,
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calc_float_step(weyfusion.choices.face_mask_blur_range),
		minimum = weyfusion.choices.face_mask_blur_range[0],
		maximum = weyfusion.choices.face_mask_blur_range[-1],
	...
	with gradio.Group():
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calc_int_step(weyfusion.choices.face_mask_padding_range),
				minimum = weyfusion.choices.face_mask_padding_range[0],
				maximum = weyfusion.choices.face_mask_padding_range[-1],
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider]:
	face_mask_types = face_mask_types or weyfusion.choices.face_mask_types
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or weyfusion.choices.face_mask_regions
