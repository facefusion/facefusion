...
from weyfusion import config, metadata, state_manager, wording
from weyfusion.common_helper import create_float_metavar, create_int_metavar, get_last
from weyfusion.execution import get_available_execution_providers
from weyfusion.filesystem import list_directory
from weyfusion.jobs import job_store
from weyfusion.processors.core import get_processors_modules
...
def create_config_path_program() -> ArgumentParser:
	...
	group_paths.add_argument('--config-path', help = wording.get('help.config_path'), default = 'weyfusion.ini')
...
def create_face_detector_program() -> ArgumentParser:
	...
	group_face_detector.add_argument('--face-detector-model', help = wording.get('help.face_detector_model'), default = config.get_str_value('face_detector.face_detector_model', 'yoloface'), choices = weyfusion.choices.face_detector_models)
	known_args, _ = program.parse_known_args()
	face_detector_size_choices = weyfusion.choices.face_detector_set.get(known_args.face_detector_model)
	...
	group_face_detector.add_argument('--face-detector-angles', help = wording.get('help.face_detector_angles'), type = int, default = config.get_int_list('face_detector.face_detector_angles', '0'), choices = weyfusion.choices.face_detector_angles, nargs = '+', metavar = 'FACE_DETECTOR_ANGLES')
	group_face_detector.add_argument('--face-detector-score', help = wording.get('help.face_detector_score'), type = float, default = config.get_float_value('face_detector.face_detector_score', '0.5'), choices = weyfusion.choices.face_detector_score_range, metavar = create_float_metavar(weyfusion.choices.face_detector_score_range))
...
def create_face_landmarker_program() -> ArgumentParser:
	...
	group_face_landmarker.add_argument('--face-landmarker-model', help = wording.get('help.face_landmarker_model'), default = config.get_str_value('face_landmarker.face_landmarker_model', '2dfan4'), choices = weyfusion.choices.face_landmarker_models)
	group_face_landmarker.add_argument('--face-landmarker-score', help = wording.get('help.face_landmarker_score'), type = float, default = config.get_float_value('face_landmarker.face_landmarker_score', '0.5'), choices = weyfusion.choices.face_landmarker_score_range, metavar = create_float_metavar(weyfusion.choices.face_landmarker_score_range))
...
def create_face_selector_program() -> ArgumentParser:
	...
	group_face_selector.add_argument('--face-selector-mode', help = wording.get('help.face_selector_mode'), default = config.get_str_value('face_selector.face_selector_mode', 'reference'), choices = weyfusion.choices.face_selector_modes)
	group_face_selector.add_argument('--face-selector-order', help = wording.get('help.face_selector_order'), default = config.get_str_value('face_selector.face_selector_order', 'large-small'), choices = weyfusion.choices.face_selector_orders)
	group_face_selector.add_argument('--face-selector-age-start', help = wording.get('help.face_selector_age_start'), type = int, default = config.get_int_value('face_selector.face_selector_age_start'), choices = weyfusion.choices.face_selector_age_range, metavar = create_int_metavar(weyfusion.choices.face_selector_age_range))
	group_face_selector.add_argument('--face-selector-age-end', help = wording.get('help.face_selector_age_end'), type = int, default = config.get_int_value('face_selector.face_selector_age_end'), choices = weyfusion.choices.face_selector_age_range, metavar = create_int_metavar(weyfusion.choices.face_selector_age_range))
	group_face_selector.add_argument('--face-selector-gender', help = wording.get('help.face_selector_gender'), default = config.get_str_value('face_selector.face_selector_gender'), choices = weyfusion.choices.face_selector_genders)
	group_face_selector.add_argument('--face-selector-race', help = wording.get('help.face_selector_race'), default = config.get_str_value('face_selector.face_selector_race'), choices = weyfusion.choices.face_selector_races)
	...
	group_face_selector.add_argument('--reference-face-distance', help = wording.get('help.reference_face_distance'), type = float, default = config.get_float_value('face_selector.reference_face_distance', '0.6'), choices = weyfusion.choices.reference_face_distance_range, metavar = create_float_metavar(weyfusion.choices.reference_face_distance_range))
...
def create_face_masker_program() -> ArgumentParser:
	...
	group_face_masker.add_argument('--face-occluder-model', help = wording.get('help.face_occluder_model'), default = config.get_str_value('face_detector.face_occluder_model', 'xseg_1'), choices = weyfusion.choices.face_occluder_models)
	group_face_masker.add_argument('--face-parser-model', help = wording.get('help.face_parser_model'), default = config.get_str_value('face_detector.face_parser_model', 'bisenet_resnet_34'), choices = weyfusion.choices.face_parser_models)
	group_face_masker.add_argument('--face-mask-types', help = wording.get('help.face_mask_types').format(choices = ', '.join(weyfusion.choices.face_mask_types)), default = config.get_str_list('face_masker.face_mask_types', 'box'), choices = weyfusion.choices.face_mask_types, nargs = '+', metavar = 'FACE_MASK_TYPES')
	group_face_masker.add_argument('--face-mask-blur', help = wording.get('help.face_mask_blur'), type = float, default = config.get_float_value('face_masker.face_mask_blur', '0.3'), choices = weyfusion.choices.face_mask_blur_range, metavar = create_float_metavar(weyfusion.choices.face_mask_blur_range))
	...
	group_face_masker.add_argument('--face-mask-regions', help = wording.get('help.face_mask_regions').format(choices = ', '.join(weyfusion.choices.face_mask_regions)), default = config.get_str_list('face_masker.face_mask_regions', ' '.join(weyfusion.choices.face_mask_regions)), choices = weyfusion.choices.face_mask_regions, nargs = '+', metavar = 'FACE_MASK_REGIONS')
...
def create_frame_extraction_program() -> ArgumentParser:
	...
	group_frame_extraction.add_argument('--trim-frame-start', help = wording.get('help.trim_frame_start'), type = int, default = weyfusion.config.get_int_value('frame_extraction.trim_frame_start'))
	group_frame_extraction.add_argument('--trim-frame-end',	help = wording.get('help.trim_frame_end'), type = int, default = weyfusion.config.get_int_value('frame_extraction.trim_frame_end'))
	group_frame_extraction.add_argument('--temp-frame-format', help = wording.get('help.temp_frame_format'), default = config.get_str_value('frame_extraction.temp_frame_format', 'png'), choices = weyfusion.choices.temp_frame_formats)
...
def create_output_creation_program() -> ArgumentParser:
	...
	group_output_creation.add_argument('--output-audio-encoder', help = wording.get('help.output_audio_encoder'), default = config.get_str_value('output_creation.output_audio_encoder', 'aac'), choices = weyfusion.choices.output_audio_encoders)
	group_output_creation.add_argument('--output-video-encoder', help = wording.get('help.output_video_encoder'), default = config.get_str_value('output_creation.output_video_encoder', 'libx264'), choices = weyfusion.choices.output_video_encoders)
	group_output_creation.add_argument('--output-video-preset', help = wording.get('help.output_video_preset'), default = config.get_str_value('output_creation.output_video_preset', 'veryfast'), choices = weyfusion.choices.output_video_presets)
	group_output_creation.add_argument('--output-video-quality', help = wording.get('help.output_video_quality'), type = int, default = config.get_int_value('output_creation.output_video_quality', '80'), choices = weyfusion.choices.output_video_quality_range, metavar = create_int_metavar(weyfusion.choices.output_video_quality_range))
...
def create_processors_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	available_processors = [ file.get('name') for file in list_directory('weyfusion/processors/modules') ]
...
def create_uis_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	available_ui_layouts = [ file.get('name') for file in list_directory('weyfusion/uis/layouts') ]
	...
	group_uis.add_argument('--ui-workflow', help = wording.get('help.ui_workflow'), default = config.get_str_value('uis.ui_workflow', 'instant_runner'), choices = weyfusion.choices.ui_workflows)
	return program
...
def create_execution_program() -> ArgumentParser:
	...
	group_execution.add_argument('--execution-thread-count', help = wording.get('help.execution_thread_count'), type = int, default = config.get_int_value('execution.execution_thread_count', '4'), choices = weyfusion.choices.execution_thread_count_range, metavar = create_int_metavar(weyfusion.choices.execution_thread_count_range))
	group_execution.add_argument('--execution-queue-count', help = wording.get('help.execution_queue_count'), type = int, default = config.get_int_value('execution.execution_queue_count', '1'), choices = weyfusion.choices.execution_queue_count_range, metavar = create_int_metavar(weyfusion.choices.execution_queue_count_range))
...
def create_download_providers_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	download_providers = list(weyfusion.choices.download_provider_set.keys())
	group_download = program.add_argument_group('download')
	group_download.add_argument('--download-providers', help = wording.get('help.download_providers').format(choices = ', '.join(download_providers)), default = config.get_str_list('download.download_providers', ' '.join(weyfusion.choices.download_providers)), choices = download_providers, nargs = '+', metavar = 'DOWNLOAD_PROVIDERS')
...
def create_download_scope_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	group_download = program.add_argument_group('download')
	group_download.add_argument('--download-scope', help = wording.get('help.download_scope'), default = config.get_str_value('download.download_scope', 'lite'), choices = weyfusion.choices.download_scopes)
...
def create_memory_program() -> ArgumentParser:
	...
	group_memory.add_argument('--video-memory-strategy', help = wording.get('help.video_memory_strategy'), default = config.get_str_value('memory.video_memory_strategy', 'strict'), choices = weyfusion.choices.video_memory_strategies)
	group_memory.add_argument('--system-memory-limit', help = wording.get('help.system_memory_limit'), type = int, default = config.get_int_value('memory.system_memory_limit', '0'), choices = weyfusion.choices.system_memory_limit_range, metavar = create_int_metavar(weyfusion.choices.system_memory_limit_range))
...
def create_misc_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	log_level_keys = list(weyfusion.choices.log_level_set.keys())
...
def create_job_status_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	program.add_argument('job_status', help = wording.get('help.job_status'), choices = weyfusion.choices.job_statuses)
	return program
