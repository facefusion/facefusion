...
def extract_frames(target_path: str, fps: float) -> bool:
    ...
    if trim_frame_start is not None and trim_frame_end is not None:
        ...
        commands.extend(
            ["-vf", "trim=start_frame=" + str(trim_frame_start) + ",fps=" + str(fps)]
        )
    elif trim_frame_end is not None:
        commands.extend(
            ["-vf", "trim=end_frame=" + str(trim_frame_end) + ",fps=" + str(fps)]
        )
    else:
...
def compress_image(output_path: str) -> bool:
    output_image_compression = round(
        31 - (facefusion.globals.output_image_quality * 0.31)
    )
    commands = [
        "-hwaccel",
        "auto",
...
def merge_video(target_path: str, fps: float) -> bool:
    ...
    if facefusion.globals.output_video_encoder in ["libx264", "libx265"]:
        output_video_compression = round(
            51 - (facefusion.globals.output_video_quality * 0.51)
        )
        commands.extend(["-crf", str(output_video_compression)])
    if facefusion.globals.output_video_encoder in ["libvpx-vp9"]:
        output_video_compression = round(
            63 - (facefusion.globals.output_video_quality * 0.63)
        )
        commands.extend(["-crf", str(output_video_compression)])
    ...
    commands.extend(
        ["-pix_fmt", "yuv420p", "-colorspace", "bt709", "-y", temp_output_video_path]
    )
    return run_ffmpeg(commands)
...
def get_temp_frames_pattern(target_path: str, temp_frame_prefix: str) -> str:
    ...
    return os.path.join(
        temp_directory_path,
        temp_frame_prefix + "." + facefusion.globals.temp_frame_format,
    )
...
def is_video(video_path: str) -> bool:
    if is_file(video_path):
        ...
        return bool(mimetype.mime and mimetype.mime.startswith("video/"))
    return False
...
def list_module_names(path: str) -> Optional[List[str]]:
    if os.path.exists(path):
        ...
        return [
            Path(file).stem for file in files if not Path(file).stem.startswith("__")
        ]
    return None
...
def decode_execution_providers(execution_providers: List[str]) -> List[str]:
    ...
    encoded_execution_providers = encode_execution_providers(
        available_execution_providers
    )
    return [
        execution_provider
        for execution_provider, encoded_execution_provider in zip(
            available_execution_providers, encoded_execution_providers
        )
        if any(
...
def map_device(execution_providers: List[str]) -> str:
    if "CoreMLExecutionProvider" in execution_providers:
        return "mps"
    if (
        "CUDAExecutionProvider" in execution_providers
        or "ROCMExecutionProvider" in execution_providers
    ):
        return "cuda"
