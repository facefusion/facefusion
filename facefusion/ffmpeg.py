...
def get_available_encoder_set() -> EncoderSet:
	...
	while line := process.stdout.readline().decode().lower():
		if line.startswith(' a'):
			...
			if audio_encoder in facefusion.choices.audio_encoders:
				if audio_encoder not in available_encoder_set.get('audio'):
					available_encoder_set['audio'].append(audio_encoder) #type:ignore[arg-type]

		if line.startswith(' v'):
			vision_encoder = line.split()[1]

			if vision_encoder in facefusion.choices.image_encoders:
				if vision_encoder not in available_encoder_set.get('image'):
					available_encoder_set['image'].append(vision_encoder) #type:ignore[arg-type]

			if vision_encoder in facefusion.choices.video_encoders:
				if vision_encoder not in available_encoder_set.get('video'):
