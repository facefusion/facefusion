...
def extract_frames(target_path : str, fps : float) -> bool:
	...
	if trim_frame_start is not None and trim_frame_end is not None:
		commands.extend([ '-vf', 'trim=start_frame=' + str(trim_frame_start) + ':end_frame=' + str(trim_frame_end) + ',fps=' + str(facefusion.globals.output_video_fps) ])
	elif trim_frame_start is not None:
		commands.extend([ '-vf', 'trim=start_frame=' + str(trim_frame_start) + ',fps=' + str(facefusion.globals.output_video_fps) ])
	elif trim_frame_end is not None:
		commands.extend([ '-vf', 'trim=end_frame=' + str(trim_frame_end) + ',fps=' + str(facefusion.globals.output_video_fps) ])
	else:
		commands.extend([ '-vf', 'fps=' + str(facefusion.globals.output_video_fps) ])
...
def merge_video(target_path : str, fps : float) -> bool:
	...
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d')
	commands = [ '-hwaccel', 'auto', '-r', str(facefusion.globals.output_video_fps), '-i', temp_frames_pattern, '-c:v', facefusion.globals.output_video_encoder ]
...
def restore_audio(target_path : str, output_path : str) -> bool:
	...
	if trim_frame_start is not None:
		start_time = trim_frame_start / facefusion.globals.output_video_fps
		commands.extend([ '-ss', str(start_time) ])
	if trim_frame_end is not None:
		end_time = trim_frame_end / facefusion.globals.output_video_fps
