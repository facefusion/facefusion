...
from facefusion.filesystem import are_images, are_videos, is_directory, is_file, move_directory, move_file, remove_directory, remove_file
...
def finalize_steps(job_id : str) -> bool:
	...
	for output_path, temp_output_paths in output_set.items():
		...
		if are_images(temp_output_paths):
			...
			for temp_output_path in temp_output_paths:
				if is_directory(temp_output_path) and not move_directory(temp_output_path, output_path):
					return False
	return True
