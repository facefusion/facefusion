...
def merge_frames() -> ErrorCode:
	...
	output_video_fps = state_manager.get_item('output_video_fps') or state_manager.get_item('temp_fps')
	...
	if ffmpeg.merge_video(state_manager.get_item('target_path'), state_manager.get_item('output_path'), temp_video_fps, output_video_fps, output_video_resolution, trim_frame_start, trim_frame_end):
...
def conditional_restrict_video_fps() -> Fps:
	...
	if state_manager.get_item('workflow') in [ 'audio-to-image:frames', 'audio-to-image:video' ]:
		return state_manager.get_item('temp_fps')
