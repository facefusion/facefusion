...
def modify_age(target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
	if state_manager.get_item('age_modifier_model') == 'fran':
		return modify_age_with_fran(target_face, temp_vision_frame)
	if state_manager.get_item('age_modifier_model') == 'styleganex_age':
		return modify_age_with_styleganex_age(target_face, temp_vision_frame)
	return temp_vision_frame


def modify_age_with_styleganex_age(target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
...
def modify_age_with_fran(target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
	model_templates = get_model_options().get('templates')
	model_sizes = get_model_options().get('sizes')
	face_landmark_5 = target_face.landmark_set.get('5/68').copy()
	crop_vision_frame, affine_matrix = warp_face_by_face_landmark_5(temp_vision_frame, face_landmark_5, model_templates.get('target'), model_sizes.get('target'))
	...
	crop_vision_frame = numpy.concatenate([ crop_vision_frame[0], start_age_channel, end_age_channel ], axis = 0)[ numpy.newaxis, ... ]
	crop_vision_frame = forward(crop_vision_frame.astype(numpy.float32), crop_vision_frame, age_modifier_direction)
