...
def execute_jobs():
    ...
    while True:
        ...
        print(f"{PENDING_JOBS_COUNT} jobs remaining:{ENDC}\n\n\n\n")
    ...
    def reload_job_in_facefusion_edit(job):
        ...
        if len(errors) == 2:
            ...
            if not response:
                ...
                update_job_listbox()
                return
                #update code to kill current run before executing
...
def prevent_sleep():
    ...
    if os_type == 'Windows':
        ...
        print("Prevented sleep on macOS using caffeinate.")
    elif os_type == 'Linux':
        ...
        print("Prevented sleep on Linux using systemd-inhibit.")
...
def allow_sleep():
    ...
    if os_type == 'Windows':
        ...
        if 'caffeinate_process' in globals():
            ...
            print("Allowed sleep on macOS by terminating caffeinate.")
    elif os_type == 'Linux':
        if 'inhibit_process' in globals():
            inhibit_process.terminate()  # Terminate the systemd-inhibit process
            print("Allowed sleep on Linux by terminating systemd-inhibit.")
...
def run(ui : gradio.Blocks) -> None:
	...
	ui.queue(concurrency_count = concurrency_count).launch(show_api = False, inbrowser = True, share = True, quiet = False)
