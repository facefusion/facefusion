...
from facefusion.state_manager import get_state_item, set_state_item, clear_state_item
...
def render() -> None:
	...
	is_target_image = is_image(get_state_item('target_path'))
	is_target_video = is_video(get_state_item('target_path'))
	TARGET_FILE = gradio.File(
		...
		value = get_state_item('target_path') if is_target_image or is_target_video else None
	)
	target_image_args =\
	{
		'show_label': False,
	...
	if is_target_video:
		if get_file_size(get_state_item('target_path')) > FILE_SIZE_LIMIT:
			preview_vision_frame = normalize_frame_color(get_video_frame(get_state_item('target_path')))
...
def update(file : File) -> Tuple[gradio.Image, gradio.Video]:
	...
	if file and is_image(file.name):
		set_state_item('target_path', file.name)
