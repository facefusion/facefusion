...
def render() -> None:
	...
	facefusion.globals.output_path = facefusion.globals.output_path or suggest_output_path(facefusion.globals.target_path)
	OUTPUT_PATH_TEXTBOX = gradio.Textbox(
		label = wording.get('uis.output_path_textbox'),
		value = facefusion.globals.output_path,
		max_lines = 1,
		visible = is_image(facefusion.globals.target_path) or is_video(facefusion.globals.target_path)
	)
...
def update_output_audio_encoder(output_audio_encoder : OutputAudioEncoder) -> None:
	facefusion.globals.output_audio_encoder = output_audio_encoder
