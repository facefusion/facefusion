...
from facefusion.vision import create_image_resolutions, detect_image_resolution, detect_video_fps, pack_resolution
...
def listen() -> None:
	...
	OUTPUT_VIDEO_SCALE_SLIDER.release(update_output_scale_resolution, inputs = OUTPUT_VIDEO_SCALE_SLIDER)
...
def remote_update() -> Tuple[gradio.Slider, gradio.Dropdown, gradio.Dropdown, gradio.Slider, gradio.Slider, gradio.Dropdown, gradio.Dropdown, gradio.Slider, gradio.Slider, gradio.Slider]:
	if is_image(state_manager.get_item('target_path')):
		output_image_resolution = detect_image_resolution(state_manager.get_item('target_path'))
		output_image_resolutions = create_image_resolutions(output_image_resolution)
	...
	return gradio.Slider(visible = False), gradio.Dropdown(visible = False), gradio.Dropdown(visible = False), gradio.Slider(visible = False), gradio.Slider(visible = False), gradio.Dropdown(visible = False), gradio.Dropdown(visible = False), gradio.Slider(visible = False), gradio.Slider(visible = False), gradio.Slider(visible = False)
...
def update_output_scale_resolution(output_video_resolution : float) -> None:
