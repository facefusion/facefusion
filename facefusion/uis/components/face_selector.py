...
def render() -> None:
    ...
    if is_image(state_manager.get_item("target_path")):
        ...
        reference_face_gallery_options["value"] = extract_gallery_frames(
            target_vision_frame
        )
    if is_video(state_manager.get_item("target_path")):
        target_vision_frame = read_video_frame(
    ...
    with gradio.Group():
        ...
        with gradio.Row():
            face_selector_age_start = (
                state_manager.get_item("face_selector_age_start")
                or facefusion.choices.face_selector_age_range[0]
            )
            face_selector_age_end = (
                state_manager.get_item("face_selector_age_end")
                or facefusion.choices.face_selector_age_range[-1]
            )
...
def listen() -> None:
    ...
    FACE_SELECTOR_ORDER_DROPDOWN.change(
        update_face_selector_order,
        inputs=FACE_SELECTOR_ORDER_DROPDOWN,
        outputs=REFERENCE_FACE_POSITION_GALLERY,
    )
    FACE_SELECTOR_GENDER_DROPDOWN.change(
        update_face_selector_gender,
        inputs=FACE_SELECTOR_GENDER_DROPDOWN,
        outputs=REFERENCE_FACE_POSITION_GALLERY,
    )
    FACE_SELECTOR_RACE_DROPDOWN.change(
        update_face_selector_race,
        inputs=FACE_SELECTOR_RACE_DROPDOWN,
        outputs=REFERENCE_FACE_POSITION_GALLERY,
    )
    FACE_SELECTOR_AGE_RANGE_SLIDER.release(
        update_face_selector_age_range,
        inputs=FACE_SELECTOR_AGE_RANGE_SLIDER,
        outputs=REFERENCE_FACE_POSITION_GALLERY,
    )
    REFERENCE_FACE_DISTANCE_SLIDER.release(
        update_reference_face_distance, inputs=REFERENCE_FACE_DISTANCE_SLIDER
    )

    preview_frame_slider = get_ui_component("preview_frame_slider")
    if preview_frame_slider:
        REFERENCE_FACE_POSITION_GALLERY.select(
            update_reference_frame_number, inputs=preview_frame_slider
        )
    ...
    if face_detector_score_slider:
        face_detector_score_slider.release(
            update_reference_position_gallery, outputs=REFERENCE_FACE_POSITION_GALLERY
        )

    preview_frame_slider = get_ui_component("preview_frame_slider")
    if preview_frame_slider:
        for method in ["change", "release"]:
            getattr(preview_frame_slider, method)(
                update_reference_position_gallery,
...
) -> Tuple[gradio.Gallery, gradio.Slider]:
...
def update_face_selector_order(
    face_analyser_order: FaceSelectorOrder,
) -> gradio.Gallery:
...
def update_face_selector_gender(face_selector_gender: Gender) -> gradio.Gallery:
    state_manager.set_item(
        "face_selector_gender", convert_str_none(face_selector_gender)
    )
    return update_reference_position_gallery()
...
def update_face_selector_age_range(
    face_selector_age_range: Tuple[float, float],
) -> gradio.Gallery:
