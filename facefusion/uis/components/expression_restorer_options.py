...
def listen() -> None:
	...
	if processors_checkbox_group:
		processors_checkbox_group.change(remote_update, inputs = processors_checkbox_group, outputs = [ EXPRESSION_RESTORER_MODEL_DROPDOWN, EXPRESSION_RESTORER_AREAS_CHECKBOX_GROUP, EXPRESSION_RESTORER_FACTOR_SLIDER ])


def remote_update(processors : List[str]) -> Tuple[gradio.Dropdown, gradio.CheckboxGroup, gradio.Slider]:
	has_expression_restorer = 'expression_restorer' in processors
	return gradio.Dropdown(visible = has_expression_restorer), gradio.CheckboxGroup(visible = has_expression_restorer), gradio.Slider(visible = has_expression_restorer)
