...
from facefusion.uis.core import (
    get_ui_component,
    get_ui_components,
...
def render() -> None:
    ...
    if (
        ...
        and state_manager.get_item("reference_frame_number")
    ):
    ...
    if is_image(state_manager.get_item("target_path")):
        ...
        reference_vision_frame = read_static_image(
        ...
        preview_vision_frame = process_preview_frame(
            ...
            source_voice_frame,
            target_vision_frame,
            uis_choices.preview_modes[0],
            uis_choices.preview_resolutions[-1],
        )
        preview_image_options["value"] = cv2.cvtColor(
            preview_vision_frame, cv2.COLOR_BGR2RGB
        )
        preview_image_options["elem_classes"] = [
            "image-preview",
            "is-" + detect_frame_orientation(preview_vision_frame),
        ]
...
def listen() -> None:
    ...
    if preview_resolution_dropdown:
        preview_resolution_dropdown.change(
            update_preview_image,
            inputs=[
                preview_mode_dropdown,
                preview_resolution_dropdown,
    ...
    if preview_frame_slider:
        ...
        reference_face_position_gallery = get_ui_component(
            "reference_face_position_gallery"
        )
        if reference_face_position_gallery:
            reference_face_position_gallery.select(
    ...
    for ui_component in get_ui_components(
        ["source_audio", "source_image", "target_image", "target_video"]
    ):
...
def update_preview_image(
    preview_mode: PreviewMode, preview_resolution: str, frame_number: int = 0
) -> gradio.Image:
    ...
    if is_image(state_manager.get_item("target_path")):
        ...
        target_vision_frame = read_static_image(
            state_manager.get_item("target_path"), "rgba"
        )
    ...
    if is_video(state_manager.get_item("target_path")):
        ...
        temp_vision_frame = read_video_frame(
            state_manager.get_item("target_path"), frame_number
        )
...
def clear_and_update_preview_image(
    preview_mode: PreviewMode, preview_resolution: str, frame_number: int = 0
) -> gradio.Image:
    clear_static_faces()
...
def process_preview_frame(
    ...
    target_vision_frame = restrict_frame(
        target_vision_frame, unpack_resolution(preview_resolution)
    )
    ...
    temp_vision_frame = prepare_output_frame(
        target_vision_frame, temp_vision_frame, temp_vision_mask
    )
...
def create_face_by_face(
    reference_vision_frame: VisionFrame,
    target_vision_frame: VisionFrame,
    temp_vision_frame: VisionFrame,
) -> Tuple[VisionFrame, VisionFrame]:
    target_faces = select_faces(
        reference_vision_frame[:, :, :3], target_vision_frame[:, :, :3]
    )
    ...
    if target_face:
        ...
        if numpy.any(target_crop_vision_frame) and numpy.any(output_crop_vision_frame):
            ...
            target_crop_vision_frame = fit_cover_frame(
                target_crop_vision_frame, (target_crop_dimension, target_crop_dimension)
            )
            output_crop_vision_frame = fit_cover_frame(
                output_crop_vision_frame, (target_crop_dimension, target_crop_dimension)
            )
...
def prepare_output_frame(
    ...
    temp_vision_mask = temp_vision_mask.clip(
        state_manager.get_item("background_remover_color")[-1], 255
    )
    temp_vision_frame = merge_vision_mask(temp_vision_frame, temp_vision_mask)
