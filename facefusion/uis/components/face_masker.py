...
FACE_BOX_GROUP : Optional[gradio.Group] = None
FACE_REGION_GROUP : Optional[gradio.Group] = None
...
def render() -> None:
	...
	global FACE_BOX_GROUP
	global FACE_REGION_GROUP
	...
	with gradio.Group() as FACE_BOX_GROUP:
	...
	with gradio.Group(visible = 'region' in facefusion.globals.face_mask_types) as FACE_REGION_GROUP:
...
def listen() -> None:
	FACE_MASK_TYPES_CHECKBOX_GROUP.change(update_face_mask_type, inputs = FACE_MASK_TYPES_CHECKBOX_GROUP, outputs = [ FACE_MASK_TYPES_CHECKBOX_GROUP, FACE_BOX_GROUP, FACE_REGION_GROUP ])
...
def update_face_mask_type(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.Group, gradio.Group]:
	...
	return gradio.CheckboxGroup(value = face_mask_types), gradio.Group(visible = 'box' in face_mask_types), gradio.Group(visible = 'region' in face_mask_types)
