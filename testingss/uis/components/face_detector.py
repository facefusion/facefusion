...
from testingss import face_detector, state_manager, wording
from testingss.common_helper import calc_float_step, get_last
from testingss.types import Angle, FaceDetectorModel, Score
from testingss.uis.core import register_ui_component
...
def render() -> None:
	...
	if state_manager.get_item('face_detector_size') in testingss.choices.face_detector_set[state_manager.get_item('face_detector_model')]:
		face_detector_size_dropdown_options['choices'] = testingss.choices.face_detector_set[state_manager.get_item('face_detector_model')]
	with gradio.Row():
		FACE_DETECTOR_MODEL_DROPDOWN = gradio.Dropdown(
			label = wording.get('uis.face_detector_model_dropdown'),
			choices = testingss.choices.face_detector_models,
	...
	FACE_DETECTOR_ANGLES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = testingss.choices.face_detector_angles,
	...
	FACE_DETECTOR_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(testingss.choices.face_detector_score_range),
		minimum = testingss.choices.face_detector_score_range[0],
		maximum = testingss.choices.face_detector_score_range[-1]
	)
...
def update_face_detector_model(face_detector_model : FaceDetectorModel) -> Tuple[gradio.Dropdown, gradio.Dropdown]:
	...
	if face_detector.pre_check():
		face_detector_size_choices = testingss.choices.face_detector_set.get(state_manager.get_item('face_detector_model'))
...
def update_face_detector_angles(face_detector_angles : Sequence[Angle]) -> gradio.CheckboxGroup:
	face_detector_angles = face_detector_angles or testingss.choices.face_detector_angles
