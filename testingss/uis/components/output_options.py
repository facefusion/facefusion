...
from testingss.common_helper import calc_int_step
from testingss.ffmpeg import get_available_encoder_set
from testingss.filesystem import is_image, is_video
from testingss.types import AudioEncoder, Fps, VideoEncoder, VideoPreset
from testingss.uis.core import get_ui_components, register_ui_component
from testingss.vision import create_image_resolutions, create_video_resolutions, detect_image_resolution, detect_video_fps, detect_video_resolution, pack_resolution
...
def render() -> None:
	...
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		...
		step = calc_int_step(testingss.choices.output_image_quality_range),
		minimum = testingss.choices.output_image_quality_range[0],
		maximum = testingss.choices.output_image_quality_range[-1],
	...
	OUTPUT_AUDIO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calc_int_step(testingss.choices.output_audio_quality_range),
		minimum = testingss.choices.output_audio_quality_range[0],
		maximum = testingss.choices.output_audio_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_AUDIO_VOLUME_SLIDER = gradio.Slider(
		...
		step = calc_int_step(testingss.choices.output_audio_volume_range),
		minimum = testingss.choices.output_audio_volume_range[0],
		maximum = testingss.choices.output_audio_volume_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		...
		choices = testingss.choices.output_video_presets,
	...
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calc_int_step(testingss.choices.output_video_quality_range),
		minimum = testingss.choices.output_video_quality_range[0],
		maximum = testingss.choices.output_video_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
