...
from testingss import face_landmarker, state_manager, wording
from testingss.common_helper import calc_float_step
from testingss.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = testingss.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(testingss.choices.face_landmarker_score_range),
		minimum = testingss.choices.face_landmarker_score_range[0],
		maximum = testingss.choices.face_landmarker_score_range[-1]
	)
