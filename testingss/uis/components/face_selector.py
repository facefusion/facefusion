...
from testingss import state_manager, wording
from testingss.common_helper import calc_float_step, calc_int_step
from testingss.face_analyser import get_many_faces
from testingss.face_selector import sort_and_filter_faces
from testingss.face_store import clear_reference_faces, clear_static_faces
from testingss.filesystem import is_image, is_video
from testingss.types import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
from testingss.uis.core import get_ui_component, get_ui_components, register_ui_component
from testingss.uis.types import ComponentOptions
from testingss.uis.ui_helper import convert_str_none
from testingss.vision import normalize_frame_color, read_static_image, read_video_frame
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = testingss.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = testingss.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + testingss.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = ['none'] + testingss.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or testingss.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or testingss.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = wording.get('uis.face_selector_age_range_slider'),
				minimum = testingss.choices.face_selector_age_range[0],
				maximum = testingss.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calc_int_step(testingss.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calc_float_step(testingss.choices.reference_face_distance_range),
		minimum = testingss.choices.reference_face_distance_range[0],
		maximum = testingss.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
