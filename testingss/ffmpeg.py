...
from testingss import ffmpeg_builder, logger, process_manager, state_manager, wording
from testingss.filesystem import get_file_format, remove_file
from testingss.temp_helper import get_temp_file_path, get_temp_frames_pattern
from testingss.types import AudioBuffer, Commands, EncoderSet, Fps, UpdateProgress
from testingss.vision import detect_video_duration, detect_video_fps, predict_video_frame_total
...
def get_available_encoder_set() -> EncoderSet:
	...
	while line := process.stdout.readline().decode().lower():
		if line.startswith(' a'):
			...
			if audio_encoder in testingss.choices.output_audio_encoders:
				index = testingss.choices.output_audio_encoders.index(audio_encoder) #type:ignore[arg-type]
				available_encoder_set['audio'].insert(index, audio_encoder) #type:ignore[arg-type]
		if line.startswith(' v'):
			video_encoder = line.split()[1]

			if video_encoder in testingss.choices.output_video_encoders:
				index = testingss.choices.output_video_encoders.index(video_encoder) #type:ignore[arg-type]
