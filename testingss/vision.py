...
from testingss.filesystem import get_file_extension, is_image, is_video
from testingss.types import Duration, Fps, Orientation, Resolution, VisionFrame
...
def create_image_resolutions(resolution : Resolution) -> List[str]:
	...
	if resolution:
		...
		for image_template_size in testingss.choices.image_template_sizes:
...
def create_video_resolutions(resolution : Resolution) -> List[str]:
	...
	if resolution:
		width, height = resolution
		temp_resolutions.append(normalize_resolution(resolution))
		for video_template_size in testingss.choices.video_template_sizes:
			if width > height:
