...
def test_colorize_frame_to_image() -> None:
	commands = [ sys.executable, 'run.py', '-j', get_test_jobs_directory(), '--headless', '--frame-processors', 'frame_colorizer', '-t', get_test_example_file('target-240p-0sat.jpg'), '-o', get_test_output_file('test_colorize-frame-to-image.jpg') ]
...
def test_colorize_frame_to_video() -> None:
	commands = [ sys.executable, 'run.py', '-j', get_test_jobs_directory(), '--headless', '--frame-processors', 'frame_colorizer', '-t', get_test_example_file('target-240p-0sat.mp4'), '-o', get_test_output_file('test-colorize-frame-to-video.mp4'), '--trim-frame-end', '10' ]
