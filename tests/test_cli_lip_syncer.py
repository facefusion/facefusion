...
def test_sync_lip_to_image() -> None:
	commands = [ sys.executable, 'facefusion.py', '-j', get_test_jobs_directory(), '--headless', '--frame-processors', 'lip_syncer', '-s', get_test_example_file('source.mp3'), '-t', get_test_example_file('target-240p.jpg'), '-o', get_test_output_file('test_sync_lip_to_image.jpg') ]
...
def test_sync_lip_to_video() -> None:
	commands = [ sys.executable, 'facefusion.py', '-j', get_test_jobs_directory(), '--headless', '--frame-processors', 'lip_syncer', '-s', get_test_example_file('source.mp3'), '-t', get_test_example_file('target-240p.mp4'), '-o', get_test_output_file('test_sync_lip_to_video.mp4'), '--trim-frame-end', '10' ]
