...
from facefusion.ffprobe_builder import chain, run, select_audio_stream, set_error_level, set_input, set_output_key_value, set_output_value_only, show_entries
...
def test_show_entries() -> None:
	assert show_entries('stream=sample_rate') == [ '-show_entries', 'stream=sample_rate' ]
	assert show_entries('stream=channels') == [ '-show_entries', 'stream=channels' ]
	assert show_entries('stream=duration,sample_rate') == [ '-show_entries', 'stream=duration,sample_rate' ]
