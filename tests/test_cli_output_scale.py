...
def test_output_image_scale(output_image_scale : Scale, output_image_resolution : Resolution) -> None:
	...
	commands = [ sys.executable, 'facefusion.py', 'run', '--jobs-path', get_test_jobs_directory(), '--processors', 'frame_enhancer', '-t', get_test_example_file('target-240p.jpg'), '-o', output_file_path, '--output-image-scale', str(output_image_scale), '-w', 'image-to-image' ]
...
def test_output_video_scale(output_video_scale : Scale, output_video_resolution : Resolution) -> None:
	...
	commands = [ sys.executable, 'facefusion.py', 'run', '--jobs-path', get_test_jobs_directory(), '--processors', 'frame_enhancer', '-t', get_test_example_file('target-240p.mp4'), '-o', output_file_path, '--trim-frame-end', '1', '--output-video-scale', str(output_video_scale), '-w', 'image-to-video' ]
