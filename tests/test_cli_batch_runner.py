...
def test_batch_run_targets() -> None:
	commands = [ sys.executable, 'facefusion.py', 'batch-run', '--workflow', 'image-to-image', '--jobs-path', get_test_jobs_directory(), '--processors', 'face_debugger', '--target-pattern', get_test_example_file('target-240p-batch-*.jpg'), '--output-pattern', get_test_output_file('test-batch-run-targets-{index}.jpg') ]
...
def test_batch_run_sources_to_targets() -> None:
	commands = [ sys.executable, 'facefusion.py', 'batch-run', '--workflow', 'image-to-image', '--jobs-path', get_test_jobs_directory(), '--source-pattern', get_test_example_file('target-240p-batch-*.jpg'), '--target-pattern', get_test_example_file('target-240p-batch-*.jpg'), '--output-pattern', get_test_output_file('test-batch-run-sources-to-targets-{index}.jpg') ]
