...
from facefusion.apis.stream_helper import create_video_encoder, decode_video_frame, destroy_video_encoder, process_image, process_video, receive_audio_frames, receive_video_frames, receive_vision_frames, run_encode_loop, run_peer_loop, update_video_encoder_bitrate
...
def test_receive_video_frames(video_codec : VideoCodec) -> None:
	...
	with patch('facefusion.apis.stream_helper.datachannel_module.create_static_library', return_value = datachannel_library_mock), \
		...
		rtc_peer_video : RtcPeerVideo = { 'sender_track': 0, 'receiver_track': 0, 'codec': video_codec }
		receiver_thread = threading.Thread(target = receive_video_frames, args = (rtc_peer_video, video_deque, video_event), daemon = True)
...
def test_receive_audio_frames(audio_codec : AudioCodec) -> None:
	...
	with patch('facefusion.apis.stream_helper.datachannel_module.create_static_library', return_value = datachannel_library_mock), \
		...
		rtc_peer_audio : RtcPeerAudio = { 'sender_track': 0, 'receiver_track': 0, 'codec': audio_codec }
		receiver_thread = threading.Thread(target = receive_audio_frames, args = (rtc_peer_audio, audio_deque), daemon = True)
		receiver_thread.start()
		receiver_thread.join(timeout = 2.0)
