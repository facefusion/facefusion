...
def test_output_video_fps(workflow_strategy : WorkflowStrategy, output_video_fps : Fps, trim_frame_end : int, output_video_frame_total : int) -> None:
	actual_file_path = get_test_output_path('test-output-video-fps-actual-' + workflow_strategy + '-' + str(output_video_fps) + '.mp4')
	expect_file_path = get_test_output_path('test-output-video-fps-expect-' + workflow_strategy + '-' + str(output_video_fps) + '.mp4')
	actual_commands = [ sys.executable, 'facefusion.py', 'run', '--workflow-mode', 'image-to-video', '--workflow-strategy', workflow_strategy, '--jobs-path', get_test_jobs_directory(), '-s', get_test_example_file('source.jpg'), '-t', get_test_example_file('target-240p.mp4'), '-o', actual_file_path, '--trim-frame-end', str(trim_frame_end), '--output-video-fps', str(output_video_fps) ]
	expect_commands = [ sys.executable, 'facefusion.py', 'run', '--workflow-mode', 'image-to-video', '--workflow-strategy', workflow_strategy, '--jobs-path', get_test_jobs_directory(), '-s', get_test_example_file('source.jpg'), '-t', get_test_example_file('target-240p-30frames-' + str(output_video_fps) + 'fps.mp4'), '-o', expect_file_path ]
	...
	for frame_index in range(output_video_frame_total):
		actual_vision_frame = read_video_frame(actual_file_path, frame_index)
		expect_vision_frame = read_video_frame(expect_file_path, frame_index)
