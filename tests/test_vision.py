...
from facefusion.vision import calculate_histogram_difference, count_trim_frame_total, count_video_frame_total, detect_image_resolution, detect_video_duration, detect_video_fps, detect_video_resolution, match_frame_color, normalize_resolution, pack_resolution, pack_video_frames, predict_video_frame_total, read_image, read_video_frame, read_video_frames, restrict_image_resolution, restrict_trim_frame, restrict_video_fps, restrict_video_resolution, scale_resolution, unpack_resolution, write_image
...
def test_read_video_frames() -> None:
	assert len(read_video_frames(get_test_example_file('target-240p-25fps.mp4'), 1, 40)) == 40
	assert read_video_frames('invalid', 1, 40) == {}


def test_pack_video_frames() -> None:
	assert len(pack_video_frames(get_test_example_file('target-240p-25fps.mp4'), 50, 5)) == 11
	assert len(pack_video_frames(get_test_example_file('target-240p-25fps.mp4'), 1, 5)) == 7
	assert len(pack_video_frames(get_test_example_file('target-240p-25fps.mp4'), 269, 5)) == 6
	assert pack_video_frames('invalid', 50, 5) == []
