...
from facefusion.face_creator import average_face_coordinates, get_many_faces, get_one_face, refill_faces
...
def test_average_face_coordinates() -> None:
	source_vision_frame = read_static_image(get_test_example_file('source.jpg'))
	...
	assert average_face_coordinates([ face_previous, face_next ], 0.5).bounding_box.tolist() == [ 40.0, 40.0, 50.0, 50.0 ]
	assert average_face_coordinates([ face_previous, face_next ], 0.5).angle == face_next.angle
	assert average_face_coordinates([ face_previous, face_next ], 0.5).embedding is face_next.embedding
	assert average_face_coordinates([ face_previous, face_next ], 0.25).embedding is face_previous.embedding
